    ...
    def handle_herb_supply(self, block):
        ...
        if supply_type == "all_herb":
            if herb_supply.entire_supply:
                try:
                    for herb_data in herb_supply.entire_supply.copy():
                        if isinstance(herb_data, tuple) and len(herb_data) == 2:
                            herb, count = herb_data
                            if adjustment == "reduce_full":
                            ...
                            logging.warning(f"Unexpected herb data format: {herb_data}")
                except Exception as e:
                    logging.error(f"Error in handle_herb_supply: {e}")
